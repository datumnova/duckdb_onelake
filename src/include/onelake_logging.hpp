...
#define ONELAKE_LOG_WRITE_START(ctx_ptr, operation, table_name, fmt, ...)                                              \
	ONELAKE_LOG_INFO(ctx_ptr, "[write:%s] Starting operation: table=%s, " fmt, operation, table_name, ##__VA_ARGS__)

#define ONELAKE_LOG_WRITE_SUCCESS(ctx_ptr, operation, table_name, rows, fmt, ...)                                      \
	ONELAKE_LOG_INFO(ctx_ptr, "[write:%s] Operation succeeded: table=%s, rows=%llu, " fmt, operation, table_name,      \
	                 (unsigned long long)(rows), ##__VA_ARGS__)
...
#define ONELAKE_LOG_DELTA_COMMIT(ctx_ptr, table_name, version, files_added, files_removed)                             \
	ONELAKE_LOG_INFO(ctx_ptr, "[delta:commit] table=%s, version=%lld, files_added=%zu, files_removed=%zu", table_name, \
	                 (long long)(version), (size_t)(files_added), (size_t)(files_removed))
