...
vector<string> BuildRelativePaths(const string &raw_location, const string &table_name) {
    ...
    if (parts.empty()) {
        ...
        default_relative = "Tables/" + StringUtil::Join(parts, parts.size(), "/", [](const string &entry) {
            return entry;
        });
    }
...
unique_ptr<FunctionData> BindDeltaFunction(ClientContext &context, TableFunction &delta_function,
...
TableFunction OneLakeTableEntry::GetScanFunction(ClientContext &context, unique_ptr<FunctionData> &bind_data) {
    ...
    string error_summary = errors.empty() ? "no candidate paths resolved"
